...
//    qDebug() << "Ref" << data.toString();
    ...
    ret = qUncompress(QByteArray::fromBase64(data.toString().toUtf8()));
    return ret;
}
...
QColor BaseStateAbstract::readColor(QStringRef val)
{
    bool ok = false;
    QColor ret;
    uint rgb = val.toString().toUInt(&ok);

    if (!ok) {
        qCritical() << tr("Invalid color from saved state");
        return ret;
    }
